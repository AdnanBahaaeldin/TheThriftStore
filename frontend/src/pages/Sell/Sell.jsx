        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-10 flex flex-col md:flex-row gap-8 items-center animate-fade-in">
          ...
          <div className="w-full md:w-1/3 flex flex-col items-center">
            <div className="w-40 h-40 rounded-xl border-2 border-dashed border-customGreen flex items-center justify-center bg-gray-100 overflow-hidden mb-2">
              {formData.image ? (
                <img src={formData.image} alt="Preview" className="object-cover w-full h-full" onError={e => e.target.style.display='none'} />
              ) : (
              ...
              {items.map(item => (
                <div key={item.id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-3 hover:shadow-2xl transition-shadow duration-200 relative group">
                  <div className="w-full h-48 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center mb-2">
                    {item.image ? (
                      <img src={item.image} alt={item.name} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-200" onError={e => e.target.style.display='none'} />
                    ) : (
                      <PlusIcon className="h-10 w-10 text-customGreen opacity-30" />
                    )}
                  </div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${categoryColors[item.category] || 'bg-gray-100 text-gray-800'}`}>{item.category || 'Other'}</span>
                  </div>
